...
class MrpProductProduce(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        a Manufacturing Number in its moves
        """
        ...
        # Check whether the product use manufacturing numbers and needs lot or SN.
        if production.exists() and production.product_id.tracking != 'none' and production.product_id.sales_lot_activated:
            if 'finished_lot_id' in fields:
                # Creates automatically SN or Lot corresponding to the Manufacturing Number.
                if len(production.move_dest_ids.mapped('sales_lot_id')) == 1 and all(move.sales_lot_id for move in production.move_dest_ids):
                    ...
                    # If the tracking is lot then the stock.production.lot must have the same name as the manufacturing number.
                    ...
                    # If the tracking is serial then the stock.production.lot must have as name the manufacturing number + one increment.
                    # e.g: Manufacturing Number is 000001 then serial numbers should be -> 00000101, 00000102, 00000103...
                    else:
