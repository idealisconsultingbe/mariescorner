    ...
    def _compute_allowed_addresses(self):
        ...
        for order in self:
            ...
            if order.partner_id:
                ...
                order.allowed_shipping_address_ids = self.env['res.partner'].search(
                    [('type', '=', 'delivery'), ('parent_id', '=', commercial_partner.id)])
                order.allowed_invoice_address_ids = self.env['res.partner'].search(
                    [('type', '=', 'invoice'), ('parent_id', '=', commercial_partner.id)])
